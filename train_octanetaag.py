...
@click.option('--coarse-num-epochs', type=int, default=300)
@click.option('--fine-num-epochs', type=int, default=300)
@click.option('--fine-in-channels', type=int, default=9)
@click.option('--experiment-start-index', type=int, default=0)
@click.option('--experiment-end-index', type=int, default=None)
@click.option('--monitor', type=str, default='val/dice')
@click.option('--fixed-epoch', type=bool, is_flag=True, help='Disable best checkpoint and perform fixed-epoch experiments')
...
def run_rose(
    train, prepare_data, seed, coarse_num_epochs, fine_num_epochs, fine_in_channels, experiment_start_index, experiment_end_index, monitor,
    fixed_epoch, mem_gb, timeout_min, force_slurm, slurm_partition, exclude_node, concurrency, nodes, num_gpus_per_node, log_dir, base_logging_frequency, dryrun):
    ...
    from runsets.rose.experiments.experiments import OCTANetAttentionGateRoseFullyExperimentConfig, submit_octanet_experiments
    ...
    if fixed_epoch:
        monitor = None
    experimentations = OCTANetAttentionGateRoseFullyExperimentConfig(
        coarse_num_epochs=coarse_num_epochs, fine_num_epochs=fine_num_epochs,
        start_index=experiment_start_index, end_index=experiment_end_index,
        get_parallel_job=num_jobs_per_submit, fine_in_channels=fine_in_channels, base_logging_frequency=base_logging_frequency, monitor=monitor)
                ...
                if not _stop_flag and len(job_queue) < concurrency:
                    ...
                    try:
                        ...
                        while (not success) and retry > 0:
                            try:
                                job = executor.submit(submit_octanet_experiments, experiment, dry_run=dryrun)
                                success = True
