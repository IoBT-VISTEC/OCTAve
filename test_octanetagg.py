...
@click.option('--ran-vanilla', is_flag=True, help="Run OCTAnet without any modification")
...
def run_rose(
    train, prepare_data, seed, coarse_num_epochs, fine_num_epochs, ran_vanilla, experiment_start_index, experiment_end_index,
    ...
    from runsets.rose.experiments.experiments import OCTANetAttentionGateRoseFullyExperimentConfig, submit_octanet_test
    ...
    if ran_vanilla:
        experimentations = OCTANetAttentionGateRoseFullyExperimentConfig(
            ...
            gating_level=-1, ssds=False, fine_in_channels=3)
    else:
        experimentations = OCTANetAttentionGateRoseFullyExperimentConfig(
            coarse_num_epochs=coarse_num_epochs, fine_num_epochs=fine_num_epochs,
            start_index=experiment_start_index, end_index=experiment_end_index,
                ...
                if not _stop_flag and len(job_queue) < concurrency:
                    ...
                    try:
                        ...
                        while (not success) and retry > 0:
                            try:
                                job = executor.submit(submit_octanet_test, experiment, dry_run=dryrun)
                                success = True
