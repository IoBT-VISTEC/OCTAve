...
@click.option('--loi', type=click.Choice(['vessel', 'faz']), help='Label of Interest', default='vessel')
@click.option('--fully-supervise', is_flag=True, help="Train in Fully-supervised setting.")
def run_octa500(
    train, prepare_data, seed, folds, scribble_presence_ratio, num_epochs, experiment_start_index, experiment_end_index,
    mem_gb, timeout_min, force_slurm, slurm_partition, exclude_node, concurrency, nodes, num_gpus_per_node, log_dir,
    base_logging_frequency, dryrun, loi, fully_supervise):
    ...
    from runsets.octa500.experiments.experiments import OCTAVEOCTA500ExperimentConfig, OCTAVEOCTA500FAZExperimentConfig, submit_octave_experiment
    ...
    logger.info(f'Experiment\'s LOI: {loi}')
    weakly_supervise = not fully_supervise
    if loi == 'vessel':
        experimentations = OCTAVEOCTA500ExperimentConfig(
            n_folds=folds, num_epochs=num_epochs,
            start_index=experiment_start_index, end_index=experiment_end_index,
            get_parallel_job=num_jobs_per_submit, base_logging_frequency=base_logging_frequency,
            scribble_presence_ratio=scribble_presence_ratio)
    elif loi == 'faz':
        experimentations = OCTAVEOCTA500FAZExperimentConfig(
            n_folds=folds, num_epochs=num_epochs,
            start_index=experiment_start_index, end_index=experiment_end_index,
            get_parallel_job=num_jobs_per_submit, base_logging_frequency=base_logging_frequency,
            scribble_presence_ratio=scribble_presence_ratio,
            ...
            while not _stop_flag:
                ...
                if not _stop_flag and len(job_queue) < concurrency:
                    try:
                        ...
                        job = executor.submit(submit_octave_experiment, experiment, dry_run=dryrun)
