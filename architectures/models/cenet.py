...
from architectures.extra.resnest import resnet34
...
class CE_Net(nn.Module):
    def __init__(self, num_classes=1, num_channels=3, model_path: str = 'resnet34-333f7ec4.pth'):
        super(CE_Net, self).__init__()
        filters = [64, 128, 256, 512]
        resnet = resnet34(pretrained=True, model_path=model_path)
    ...
    def forward(self, x):
        ...
        # Decoder
        # Padding errror fix
        d4_u = self.decoder4(e4)
        pad_size = torch.tensor(d4_u.shape[2] - e3.shape[2])
        if pad_size != 0:
            if pad_size > 0:
                e3 = F.pad(e3, (0, pad_size, 0, pad_size))
            if pad_size < 0:
                e3 = e3[..., 0:d4_u.shape[2], 0:d4_u.shape[3]]
        d4 = d4_u + e3

        d3_u = self.decoder3(d4)
        pad_size = torch.tensor(d3_u.shape[2] - e2.shape[2])
        if pad_size != 0:
            if pad_size > 0:
                e2 = F.pad(e2, (0, pad_size, 0, pad_size))
            if pad_size < 0:
                e2 = e2[..., 0:d3_u.shape[2], 0:d3_u.shape[3]]
        d3 = d3_u + e2

        d2_u = self.decoder2(d3)
        pad_size = torch.tensor(d2_u.shape[2] - e1.shape[2])
        if pad_size != 0:
            if pad_size > 0:
                e1 = F.pad(e1, (0, pad_size, 0, pad_size))
            if pad_size < 0:
                e1 = e1[..., 0:d2_u.shape[2], 0:d2_u.shape[3]]
        d2 = d2_u + e1
