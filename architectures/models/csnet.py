...
class CSNet(nn.Module):
    ...
    def concat_shape(self, d, e):
        pad_size = torch.tensor(d.shape[2] - e.shape[2])
        if pad_size != 0:
            if pad_size > 0:
                e = F.pad(e, (0, pad_size, 0, pad_size))
            if pad_size < 0:
                e = e[..., 0:d.shape[2], 0:d.shape[3]]
    ...
    def forward(self, x):
        ...
        up4 = self.concat_shape(up4, enc3)
        dec4 = self.decoder4(up4)

        up3 = self.deconv3(dec4)
        up3 = self.concat_shape(up3, enc2)
        dec3 = self.decoder3(up3)

        up2 = self.deconv2(dec3)
        up2 = self.concat_shape(up2, enc1)
        dec2 = self.decoder2(up2)

        up1 = self.deconv1(dec2)
        up1 = self.concat_shape(up1, enc_input)
